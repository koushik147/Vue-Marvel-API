...
<span v-if="Series.length !== 0">
  ...
  <div>
    <ul>
      <li v-for="(Series,index) in Series" :key="index">
        <img
          :src="Series.thumbnail.path+ '.' + Series.thumbnail.extension" alt="Image description"
        />
        <br/><br/>

        <router-link :to="{name: 'Series', params: {id: Series.id}}" class="text">
           {{Series.title}}
        </router-link>
...
export default 
  ...
  data() { 
    ...
    { 
        ...
        .then(({ data }) => {this.Series = data.data.results}); 
    },
    watch:{
      $route()
      {
